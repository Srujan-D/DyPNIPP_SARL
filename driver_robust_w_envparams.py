...
from attention_net_robust_w_envparams import AttentionNet, PredictNextBelief, EncoderSimParams
...
def writeToTensorBoard(writer, tensorboardData, curr_episode, plotMeans=True):
    ...
    if use_wandb:
        ...
        wandb.log({"Losses/Env Sim Params Loss": belief_loss}, step=curr_episode)
...
def main():
    ...
    env_sim_params = EncoderSimParams(device).to(device)
    # env_sim_params.eval()
    ...
    try:
        while True:
            ...
            while len(experience_buffer["node_inputs"]) >= BATCH_SIZE:
                ...
                env_sim_params_batch = torch.stack(rollouts["env_sim_params"])
                ...
                if device != local_device:
                    ...
                    env_sim_params_batch = env_sim_params_batch.to(device)
                ...
                for i in range(UPDATE_EPOCHS):
                    with autocast():
                        ...
                        env_sim_params_batch = env_sim_params_batch.to(device)
                        encoded_env_sim_params = env_sim_params(env_sim_params_batch)
                        belief_loss= nn.L1Loss()(pred_next_belief_batch, encoded_env_sim_params) * 100.0
                        # print("belief_loss:", belief_loss.item())
                        # print("pred_next_belief_batch:", pred_next_belief_batch)
                        # print("encoded_env_sim_params:", encoded_env_sim_params)
                        belief_loss.requires_grad = True
