...
class Env():
    def __init__(self, sample_size=500, k_size=10, start=None, destination=None, obstacle=[], budget_range=None, save_image=False, seed=None, fixed_env=None):
        ...
        # real data
        # self.data_path = 'data/air.sig995.mon.mean.nc'
        # self.data_path = '/data/srujan/research/catnipp/data/air.sig995.mon.mean.nc'
        # self.data = load_nc_data(self.data_path)
        # self.data = self.data[0, 0:self.env_size, 0:self.env_size]

        # self.data = np.load('/data/srujan/research/catnipp/data/fire_58.npy')
        # self.data = self.data[-1]
        # print(self.data.shape)
        # self.data = (self.data - np.min(self.data)) / (np.max(self.data) - np.min(self.data))

        self.data = '/data/srujan/research/catnipp/data/Burn01_IR_15310619.csv'
        # read csv file, first row is header
        self.data = pd.read_csv(self.data, header=0)['TEMP']
        self.data = self.data.to_numpy()[:272484].reshape(522, 522)
    ...
    def reset(self, seed=None):
        ...
        # underlying distribution
        self.underlying_distribution = self.data
    ...
    def step(self, next_node_index, sample_length, measurement=True):
        ...
        while remain_length > next_length:
            ...
            if measurement:
                observed_value = self.underlying_distribution[min(int(self.sample[0] * self.env_size), self.env_size - 1), min(int(self.sample[1] * self.env_size), self.env_size - 1)] #+ np.random.normal(0, 1e-10)
                # print(min(int(self.sample[0] * self.env_size), self.env_size - 1), min(int(self.sample[1] * self.env_size), self.env_size - 1), observed_value)
            else:
    ...
    def route_step(self, route, sample_length, measurement=True):
        ...
        for next_node in route[1:]:
            ...
            while remain_length > next_length:
                ...
                observed_value = self.underlying_distribution[min(int(self.sample[0, 0] * self.env_size), self.env_size - 1), min(int(self.sample[0, 1] * self.env_size), self.env_size - 1)] #+ np.random.normal(0, 1e-10)
                # print(min(int(self.sample[0] * self.env_size), self.env_size - 1), min(int(self.sample[1] * self.env_size), self.env_size - 1), observed_value)
                self.gp_ipp.add_observed_point(self.sample, observed_value)
                remain_length -= next_length
                next_length = sample_length
    ...
    def get_ground_truth(self):
        ...
        ground_truth = self.underlying_distribution.reshape(-1)
        return ground_truth
    ...
    def plot(self, route, n, step, path, testID=0, CMAES_route=False, sampling_path=False):
        ...
        plt.hist2d(xh, yh, bins=self.env_size, range=[[0,1], [0,1]], vmin=0, vmax=1, rasterized=True)
        # plt.scatter(self.start[:,0], self.start[:,1], c='r', s=15)
        # plt.scatter(self.destination[:,0], self.destination[:,1], c='r', s=15)
