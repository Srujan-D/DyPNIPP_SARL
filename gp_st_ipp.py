...
from classes.Gaussian3D import Gaussian3D
...
class GaussianProcessForIPP():
    def __init__(self, node_coords):
        ...
        # self.kernel = RBF(0.2)
        self.xy_kernel = Matern(length_scale=0.45)
        self.t_kernel = RBF(length_scale=2.0)
        self.kernel = self.xy_kernel * self.t_kernel
    ...
    def update_gp(self):
        if self.observed_points:
            X = np.array(self.observed_points).reshape(-1,3)
    ...
    def evaluate_RMSE(self, y_true):
        ...
        y_pred, std = self.gp.predict(x1x2x3, return_std=True)
    ...
    def get_high_info_area(self, t=ADAPTIVE_TH, beta=1):
        ...
        for i in range(900):
            if y_pred[i] + beta * std[i] >= t:
                high_measurement_area.append(x1x2x3[i])
    ...
    def plot(self, y_true):
        ...
        X0p, X1p = x1x2x3[:, 0].reshape(30, 30), x1x2x3[:, 1].reshape(30, 30)
        # y_pred = np.reshape(y_pred, (30, 30))
        # std = std.reshape(30,30)

        y_pred = np.reshape(y_pred, (30, 30, 10))
        std = std.reshape(30,30, 10)
        ...
        for i in range(10):
            ...
            plt.pcolormesh(X0p, X1p, y_true[i].reshape(30, 30), shading='auto', vmin=0, vmax=1)
            plt.subplot(2, 2, 3) # stddev
            plt.title('Predict std')
            plt.pcolormesh(X0p, X1p, std[i], shading='auto', vmin=0, vmax=1)
            plt.subplot(2, 2, 1) # mean
            plt.title('Predict mean')
            plt.pcolormesh(X0p, X1p, y_pred[i], shading='auto', vmin=0, vmax=1)
            plt.xlim((0, 1))
            plt.ylim((0, 1))
        # if self.observed_points:
...
if __name__ == '__main__':
    ...
    # x1x2 = np.array(list(product(x1, x2)))
    x1x2x3 = np.array(list(product(x1, x2, x3)))
    y_true = example.distribution_function(X=x1x2x3)
    # print(y_true.shape)
