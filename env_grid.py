...
from classes import PRMController, Obstacle, Utils, Grid
...
class Env():
    def __init__(self, sample_size=500, k_size=10, start=None, destination=None, obstacle=[], budget_range=None, save_image=False, seed=None, fixed_env=None, env_size=30):
        ...
        self.prm = self.grid
        self.budget = np.random.uniform(*self.budget_range)
        self.node_coords, self.graph = self.grid.runGrid()
        ...
        # self.data = self.data[0, 0:30, 0:30]
    ...
    def step(self, next_node_index, sample_length, measurement=True):
        ...
        while remain_length > next_length:
            ...
            if measurement:
                observed_value = self.underlying_distribution[min(int(self.sample[0] * 30), 29), min(int(self.sample[1] * 30), 29)] #+ np.random.normal(0, 1e-10)
                # print(min(int(self.sample[0] * 30), 29), min(int(self.sample[1] * 30), 29), observed_value)
            else:
    ...
    def route_step(self, route, sample_length, measurement=True):
        ...
        for next_node in route[1:]:
            ...
            while remain_length > next_length:
                ...
                observed_value = self.underlying_distribution[min(int(self.sample[0, 0] * 30), 29), min(int(self.sample[0, 1] * 30), 29)] #+ np.random.normal(0, 1e-10)
                # print(min(int(self.sample[0] * 30), 29), min(int(self.sample[1] * 30), 29), observed_value)
                self.gp_ipp.add_observed_point(self.sample, observed_value)
                remain_length -= next_length
                next_length = sample_length
    ...
    def plot(self, route, n, step, path, testID=0, CMAES_route=False, sampling_path=False):
        ...
        if CMAES_route:
            ...
            pointsToDisplay = [(self.grid.findPointsFromNode(path)) for path in route]
        ...
        if sampling_path:
            pointsToDisplay2 = [(self.grid.findPointsFromNode(path)) for path in sampling_path]
