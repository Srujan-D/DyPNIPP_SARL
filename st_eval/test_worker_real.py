...
class WorkerTestReal:
    ...
    def run_episode(self, currEpisode, testID, model_idx=0):
        ...
        # save gif
        if self.save_image:
            self.make_gif(result_path, currEpisode, testID, perf_metrics["cum_rmse"])
        return perf_metrics
    ...
    def make_gif(self, path, n, testID, avg_rmse=None):
        with imageio.get_writer(
            "{}/ep{}_test{}_seed{}_winddir{}_rmse_{:.4g}.gif".format(
                path, n, testID, self.env.seed, self.env.fire.fire_info[1][3], self.env.RMSE if avg_rmse is None else avg_rmse
            ),
            mode="I",
            duration=0.05,
