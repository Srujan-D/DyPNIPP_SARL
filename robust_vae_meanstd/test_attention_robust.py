...
class PredictNextBelief(nn.Module):
    def __init__(self, device="cuda"):
        ...
        self.features_file = 'features_5fires_on5_10.npy'
        self.labels_file = 'labels_5fires_on5_10.npy'
    ...
    def create_policy_feature(self, fuel, seed):
        self.features_file = f'features_vae_nolambda_{fuel}_seed{seed}.npy'
        self.labels_file = f'labels_vae_nolambda_{fuel}_seed{seed}.npy'
        if not os.path.exists(self.features_file):
            np.save(self.features_file, np.empty((0, BELIEF_EMBEDDING_DIM), dtype=np.float32))
        if not os.path.exists(self.labels_file):
            np.save(self.labels_file, np.empty((0,), dtype=np.float32))
