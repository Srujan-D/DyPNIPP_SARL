...
class Env():
    ...
    def reset(self, seed=None):
        ...
        # underlying distribution
        self.underlying_distribution = Gaussian3D()
        ...
        while remain_length > next_length:
            ...
            if measurement:
                observed_value = self.underlying_distribution.distribution_function(
                    self.sample.reshape(-1, 3)) + np.random.normal(0, 1e-10)
            else:
    ...
    def route_step(self, route, sample_length, measurement=True):
        ...
        for next_node in route[1:]:
            ...
            while remain_length > next_length:
                ...
                observed_value = self.underlying_distribution.distribution_function(self.sample.reshape(-1, 3)) + np.random.normal(0, 1e-10)
                self.gp_ipp.add_observed_point(self.sample, observed_value)
                remain_length -= next_length
                next_length = sample_length
                no_sample = False
    ...
    def get_ground_truth(self):
        ...
        ground_truth = self.underlying_distribution.distribution_function(x1x2x3)
        return ground_truth
