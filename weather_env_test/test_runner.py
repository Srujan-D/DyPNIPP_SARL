...
# from test_worker import WorkerTest as Worker
from test_worker_real import WorkerTestReal as Worker
...
class Runner(object):
    ...
    def __init__(self, metaAgentID, multigamma=False):
        self.metaAgentID = metaAgentID
        self.device = torch.device("cuda") if USE_GPU else torch.device("cpu")
        self.localNetwork = AttentionNet(INPUT_DIM, EMBEDDING_DIM, self.device)
        self.localNetwork.to(self.device)

        self.belief_predictor = PredictNextBelief(self.device)
        self.belief_predictor.to(self.device)

        self.encoder_sim_params = EncoderSimParams(self.device)
        self.encoder_sim_params.to(self.device)
    ...
    def get_encoder_sim_params_weights(self):
        return self.encoder_sim_params.state_dict()

    def set_encoder_sim_params_weights(self, weights):
        self.encoder_sim_params.load_state_dict(weights)
    ...
    def singleThreadedJob(
        ...
        worker = Worker(
            ...
            encoder_sim_params=self.encoder_sim_params,
        )
    ...
    def job(
        ...
        encoder_sim_params_weights=None,
    ):
        print(
        ...
        if encoder_sim_params_weights is not None:
            self.set_encoder_sim_params_weights(encoder_sim_params_weights)
