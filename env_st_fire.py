...
class Env:
    def __init__(
        ...
        self.graph_ctrl = PRMController(
            self.graph_size, self.obstacles, self.start, self.destination, self.k_size
        )
    ...
    def step(
        ...
        while remain_length > next_length:
            ...
            self.underlying_distrib.single_agent_state_update(
                self.sample.reshape(-1, 2)
            )
            (
                fire_state,
    ...
    def get_ground_truth(self, scale=1):
        ...
        ground_truth = griddata(
            (x_fire_scaled, y_fire_scaled), fire_intensity, x1x2, method="linear"
        )
    ...
    def plot(
        ...
        plt.suptitle(
            "Budget: {:.4g}/{:.4g},  Cov trace: {:.4g}".format(
