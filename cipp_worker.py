...
class CIPPWorker:
    ...
    def object_function(self, X):
        ...
        if len(self.temp_observed_points) > 0:
            self.env.gp_wrapper.add_observed_points(
                deepcopy(self.temp_observed_points),
                [[deepcopy(self.temp_observed_value)]],
            )
    ...
    def budget_penalty(self, X):
        ...
        dist = np.linalg.norm(
            complete_route[1:, :] - complete_route[:-1, :], axis=-1
        ).sum()
    ...
    def cmaes_global_plan(self):
        x1 = np.linspace(self.env.start[0, 0], self.env.destination[0, 0], 50).reshape(
            -1, 1
        )
        x2 = np.linspace(self.env.start[0, 1], self.env.destination[0, 1], 50).reshape(
            -1, 1
        )
        ...
        es = cma.fmin(
            ...
            control_points,
            0.25,
            options={"maxiter": 500, "bounds": [0, 1], "popsize": 50, "tolfun": 0.1},
        )
        planned_points = es[0].reshape(-1, 2)
        planned_route = np.concatenate(
            (self.env.start, planned_points, self.env.destination)
        )
        ...
        high_info = self.env.gp_wrapper.get_high_info_area()
        _, cov_trace = self.env.gp_wrapper.eval_avg_cov_trace(
            self.env.curr_t, high_info, return_all=True
        )
        print(cov_trace)
        return cov_trace
    ...
    def cmaes_plan(self, start, end):
        ...
        self.temp_observed_points = self.env.gp_wrapper.GPs[0].observed_points
        self.temp_observed_value = self.env.gp_wrapper.GPs[0].observed_value
        ...
        es = cma.fmin(
            ...
            control_points,
            0.25,
            options={"maxiter": 100, "bounds": [0, 1], "popsize": 10},
        )
        planned_points = es[0].reshape(-1, 2)
        planned_route = np.concatenate((start, planned_points, end))

        self.env.gp_wrapper.add_observed_points(
            self.temp_observed_points, self.temp_observed_value
        )
    ...
    def adaptive_cmaes_plan(self):
        ...
        while True:
            ...
            if travel_dist / (1 + self.budget_factor) + self.horizon > pg_length:
                ...
                self.end = (
                    global_start
                    + (global_destination - global_start)
                    * (travel_dist / (1 + self.budget_factor) + self.horizon)
                    / pg_length
                )
                self.next_iter_end = (
                    global_start
                    + (global_destination - global_start)
                    * (travel_dist / (1 + self.budget_factor) + dtr_g + self.horizon)
                    / pg_length
                )
            ...
            if self.save_image:
                ...
                self.env.plot(
                    real_path, self.global_step, i, result_path, CMAES_route=True
                )
            ...
            if self.local_budget <= dtr_g:
                ...
                self.execute_path(
                    np.concatenate(
                        (real_path[-1].reshape(1, 2), global_destination.reshape(1, 2))
                    )
                )
                ...
                break
        high_info = self.env.gp_wrapper.get_high_info_area(self.env.curr_t)
...
if __name__ == "__main__":
    ...
    for j in range(10):
        ...
        if SAVE_CSV_RESULT:
            ...
            results1 = []
            csv_filename = (
                f"result/CSV3/Budget_" + str(budget) + "_CMAES_planning_time.csv"
            )
