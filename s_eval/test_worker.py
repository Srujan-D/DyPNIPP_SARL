...
class WorkerTest:
    ...
    def run_episode(self, currEpisode, testID, model_idx=0):
        ...
        if self.multigamma:
            ...
            elif SPECIFIC_GAMMA is not None:
                idx = SPECIFIC_GAMMA
            elif DECREASE_GAMMA:
                idx = len(MULTI_GAMMA) - 1
        ...
        for i in range(256):
            ...
            reward, done, node_info, node_std, remain_budget, effective_gamma = (
                self.env.step(next_node_index.item(), self.sample_length)
            )
            if FIT_GAMMA:
                idx = np.abs(np.array(MULTI_GAMMA) - effective_gamma).argmin()
                # print("effective_gamma : ", effective_gamma,  "   idx ---- : ", idx)
            ...
            # print(node_inputs)
            c_budget = torch.gather(budget_inputs, 1, current_index).cpu().item()

            if DECREASE_GAMMA:
                if remain_budget > 8:
                    idx = len(MULTI_GAMMA) - 1
                elif remain_budget > 6:
                    idx = len(MULTI_GAMMA) - 2
                elif remain_budget > 4:
                    idx = len(MULTI_GAMMA) - 3
                elif remain_budget > 3:
                    idx = len(MULTI_GAMMA) - 4
                elif remain_budget > 2:
                    idx = len(MULTI_GAMMA) - 5
                else:
                    idx = 0
                if idx < 0:
                    idx = 0
            ...
            if done:
                # print("#####  Don e  ! ! ! ")
                # print("self.env.current_node_index ; ", self.env.current_node_index)
                if self.env.current_node_index == 0:
                    perf_metrics["budget"] = budget
                    perf_metrics["RMSE"] = self.env.gp_ipp.evaluate_RMSE(
                    ...
                    perf_metrics["MI"] = self.env.gp_ipp.evaluate_mutual_info(
                        self.env.high_info_area
                    )
        ...
        # save gif
        if self.save_image:
            self.make_gif(result_path[model_idx], currEpisode, testID)
        return perf_metrics
    ...
    def run_trajectory_sampling_episode(self, currEpisode, testID):
        ...
        for i in range(256):  # 256
            ...
            for next_node_index in best_route[:3]:
                ...
                _, done, node_info, node_std, remain_budget = self.env.step(
    ...
    def plan_route(
        ...
        for j in range(PLAN_STEP):
            ...
            _, done, _, node_std, remain_budget = env.step(
    ...
    def make_gif(self, path, n, testID):
        with imageio.get_writer(
            "{}/ep{}_test{}_cov_trace_{:.4g}.gif".format(
                path, n, testID, self.env.cov_trace
            ),
            mode="I",
            duration=0.5,
        ) as writer:
