...
class RAOr:
    def __init__(self, sample_size=100, seed=None, show_img=False, env=None):
        ...
        self.start, self.end = (
            self.env.start.squeeze() * self.magnify,
            self.env.destination.squeeze() * self.magnify,
        )
    ...
    def sample_uniform(self, exist_route):
        ...
        while True:
            random_idx = np.random.choice(
                range(1, len(self.coords)), size=num_sample_nodes, replace=False
            ).tolist()
            ...
            for i in random_idx:
                ...
                dist = self.env.prm.calcDistance(
                    coord, start_coord
                ) + self.env.prm.calcDistance(coord, end_coord)
    ...
    def clean_coords(self, idx_pre):
        ...
        self.sample_idx = [0, start_idx] + list(set(idx_pre))[
            1:
        ]  # set will start with 0
        for idx in self.sample_idx:
            self.tsp_coords.append(self.coords[idx].squeeze())
        return
    ...
    def new_sample(self, graph_idx):
        ...
        v_new = np.random.choice(
            range(2, num_nodes + 2), size=num_sample, replace=False
        ).tolist()
        for v in v_new:
            if v in graph_idx:
    ...
    def calc_prob(self):
        ...
        y_pred, y_std = self.env.gp_wrapper.return_grid()
    ...
    def new_sample_adaptive(self, graph_idx, exist_route):
        ...
        while graph_idx0 == graph_idx:
            ...
            while flag:
                ...
                v_new = np.random.choice(
                    range(1, num_nodes + 2), size=num_sample, replace=False
                ).tolist()
                for v in v_new:
    ...
    def calc_budget(self, graph_idx, used_budget):
        ...
        budget_limit = min(
            self.budget0 - used_budget, self.horizon * (1 + self.budget_factor)
        )
    ...
    def get_cov_trace(self, route, route_ahead=[], measurement=True):
        ...
        cov_trace, rmse = env.route_step(
            true_coords, self.sample_length, measurement=True
        )
        if route_ahead:
            true_coords_ahead = []
            ...
            cov_trace, rmse = env.route_step(
                true_coords_ahead, self.sample_length, measurement=False
            )
    ...
    def plot(self, route, n1, n2):
        ...
        self.env.gp_wrapper.plot(self.env.ground_truth, self.env.curr_t)
        # plt.subplot(1, 3, 1)
        ...
        #     plt.scatter(self.env.start[:, 0], self.env.start[:, 1], c="r", s=15)
        #     plt.scatter(
        #         self.env.destination[:, 0], self.env.destination[:, 1], c="r", s=15
        #     )

        plt.suptitle(
            "Budget: {:.4g}/{:.4g},  RMSE: {:.4g}, CovTrace:{:.4g}".format(
                self.env.budget,
                self.budget0,
                self.env.gp_wrapper.eval_avg_RMSE(
                    self.env.ground_truth, self.env.curr_t
                ),
                # self.env.gp_ipp.evaluate_F1score(self.env.ground_truth),
                self.env.cov_trace,
            )
        )
    ...
    def make_gif(self, n):
        with imageio.get_writer(
            "{}/{}_cov_trace_{:.4g}.gif".format(self.path, n, self.env.cov_trace),
            mode="I",
            duration=0.5,
        ) as writer:
    ...
    def run_RAOr(self, exist_route=[1], used_budget=0):
        ...
        while True:
            ...
            cov_trace_best, rmse = self.get_cov_trace(
                exist_route, route, measurement=False
            )
        ...
        for cnt in range(
            (self.coords.__len__() - exist_route.__len__()) * self.iter_times
        ):
            ...
            if remain_budget < 0:
                ...
                #     break
                continue
            cov_trace, rmse = self.get_cov_trace(exist_route, route, measurement=False)
        ...
        return cov_trace_best, route_best, rmse
    ...
    def run_RAOr_adaptive(self):
        ...
        while True:
            ...
            for i in range(len(route) - 1):
                ...
                print("Dist:", dist, self.dtr)
                n_iter_new = dist // self.dtr
                if n_iter_new != n_iter:
                    n_iter = n_iter_new
                    final_route += route[1 : i + 2]
                    cov, rmse = self.get_cov_trace(final_route, measurement=True)
                    print(">>> 1st break")
                    break
                    ...
                    print(">>> 2nd break")
                    break
            cov1, route, rmse1 = self.run_RAOr(
                exist_route=final_route, used_budget=dist
            )
            cov_list.append(cov)
            rmse_list.append(rmse)
            avg_rmse = self.env.gp_wrapper.eval_avg_RMSE(
                self.env.ground_truth, self.env.curr_t
            )
            self.budget_history.append(dist)
            self.obj_history.append([cov, avg_rmse])
            print("Main loop info:", cnt, dist, cov, cov1, rmse, rmse1, final_route)
            if 0 in final_route:
                print(">>> 3rd break")
                break
        if self.show_img:
        ...
        print("cum RMSE:", np.sum(rmse_list))
        print("avg rmse:", avg_rmse)
        print("Route:", final_route)
        print("Iter times:", cnt)
        return cov, final_route, rmse_list
...
if __name__ == "__main__":
    ...
    for j in range(NUM_REPEAT):
        ...
        for i in range(100):
            try:
                def fire_step_thread(env: Env):
                    init_gt = env.ground_truth.reshape(30, 30)
                    index = 0
                    while True:
                        # print("Fire step:", index)
                        (
                            _,
                            _,
                            _,
                        ...
                        ) = env.fire.env_step(r_func="RF4")
                        gt = Utils.compress_and_average(interp_fire_intensity, (30,30))
                        init_gt = 0.9 * init_gt + 0.1 * gt
                        del gt
                        # plt.imshow(init_gt, cmap="hot", origin="lower")
                        # plt.savefig(f"ex/{index}.png")

                        # plt.imshow(env.ground_truth, cmap="hot", origin="lower")
                        # plt.savefig(f"im/{index}.png")
                ...
                raor = RAOr(sample_size=200, seed=seed, show_img=False, env=env)

                fire_thread = threading.Thread(target=fire_step_thread, args=(env,))
                fire_thread.daemon = True
                fire_thread.start()

                covtr, _, rmse_list = raor.run_RAOr_adaptive()
                print("Done:", covtr, np.sum(rmse_list))
                sub_results.append(covtr)
                cum_rmse.append(np.sum(rmse_list))
                ...
                if SAVE_TRAJECTORY_HISTORY:
                    ...
                    with open(csv_filename2, "a") as csvfile:
                        ...
                        csv_data = np.concatenate(
                            (budget_history.reshape(-1, 1), obj_history.reshape(-1, 2)),
                            axis=-1,
                        )
    ...
    print("Cum RMSE:", cum_rmse)
    print("Mean cumRMSE:", np.mean(cum_rmse))
    print("Stddev cumRMSE:", np.std(cum_rmse))
    print("Avg RMSE:", avg_rmse)
    print("Mean avgRMSE:", np.mean(avg_rmse))
    print("Stddev avgRMSE:", np.std(avg_rmse))
