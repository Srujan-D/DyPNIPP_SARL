...
from load_weather_data.dataset import get_data
...
class Env:
    def __init__(
        ...
        self.n_agents = 1
        self.env_size = 30
        # # generate weather environment
        # print(">>> Generating weather environment")
        self.weather_data, self.weather_data_mean = get_data()
        # print(">>> Weather environment generated", self.weather_data.shape)
        # print(">>> Weather environment generated")
    ...
    def reset(self, seed=None):
        ...
        self.ground_truth = self.episode_weather_data
    ...
    def convert_coords2world(self, pose):
        ...
        return [int(pose_[0] * self.env_size), int(pose_[1] * self.env_size)]
    ...
    def step(self, next_node_index, sample_length, measurement=True, eval_speed=None):
        ...
        while remain_length > next_length:
            ...
            if measurement:
                sample = self.convert_coords2world(self.sample)
                observed_value = self.ground_truth[sample[0] * self.env_size + sample[1]]
            ...
            self.upadate_data_step()
            self.set_ground_truth(self.episode_weather_data)
    ...
    def upadate_data_step(self):
        self.time_offset += 1
        self.episode_weather_data = self.weather_data[self.time_offset]
        # self.episode_weather_data_mean = self.weather_data_mean[self.time_offset]

        # self.underlying_distribution = self.episode_weather_data
    ...
    def set_ground_truth(self, weather_map):
        ground_truth = Utils.compress_and_average(array=weather_map, new_shape=(30, 30))
        self.ground_truth = ground_truth.reshape(-1)
        del ground_truth

    def set_momentum_GT(self, weather_map):
        ground_truth = Utils.compress_and_average(array=weather_map, new_shape=(30, 30))
    ...
    def plot(
        ...
        plt.tight_layout()
        # extract first subfigure for later use
        sub_fig1 = plt.gcf().get_axes()[0]
        plt.savefig(
        ...
        if attention_weights is not None:
            # query figure which is same as plt subfigure 0
            # plot 1x5 subfigures with 0th being the query, and 1-4 being the attention weights
            plt.figure()
            plt.subplot(1, 5, 1)
            plt.title("Query v/s Attention")
            plt.scatter(self.start[:, 0], self.start[:, 1], c="r", s=15)
            plt.scatter(self.destination[:, 0], self.destination[:, 1], c="g", s=15)
            # same as plt subfigure 0
            plt.sca(sub_fig1)
