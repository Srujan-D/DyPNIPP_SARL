...
if __name__ == '__main__':
    ...
    device = torch.device(f'cuda:{CUDA_DEVICE[0]}') if USE_GPU_GLOBAL else torch.device('cpu')
    local_device = torch.device(f'cuda:{CUDA_DEVICE[0]}') if USE_GPU else torch.device('cpu')
    print(model_path)
    result_cov_all = []
