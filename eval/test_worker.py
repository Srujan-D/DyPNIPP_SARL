...
class WorkerTest:
    def __init__(self, metaAgentID, localNetwork, global_step, budget_range, sample_length=None, device='cuda', greedy=False, save_image=True, seed=None, multigamma=False):
    ...
    def run_episode(self, currEpisode, testID, model_idx=0):
        ...
        for i in range(256):
            ...
            # save a frame
            if self.save_image:
                if not os.path.exists(result_path[model_idx]):
                    os.makedirs(result_path[model_idx])
                self.env.plot(route, self.global_step, i, result_path[model_idx], testID)
