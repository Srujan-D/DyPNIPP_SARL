...
class EncoderSimParams(nn.Module):
    ...
    def normalize(self, x):
        x[:, 0] = (x[:, 0] - 5) / 5  # Normalize wind speed (range 0-10) and mean center
        x[:, 1] = (x[:, 1] - 5) / 5  # Normalize azimuth (range 0-10) and mean center
        x[:, 2] = (x[:, 2] - 180) / 180  # Normalize fuel coefficient (range 0-360) and mean center
        return x
