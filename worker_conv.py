...
class Worker:
    def __init__(
        ...
        self.meta_id = metaAgentID
        self.device = device
        self.greedy = greedy
        self.global_step = global_step
        self.save_image = save_image
        self.sample_size = sample_size
        self.graph_size = sample_size
        self.history_size = history_size
        self.sample_length = sample_length
        self.n_agents = target_size
        ...
        #     kernel_size=HISTORY_STRIDE, stride=HISTORY_STRIDE, ceil_mode=True
        # )
    ...
    def reset_env_input(self):
        ...
        history_pool_inputs = (
            node_history.permute(1, 2, 0).permute(2, 0, 1).unsqueeze(0)
        )  # (1, hpool, n, 2+targetxfeature)
        ...
        #     0, 2, 1
        # )  # (1, hpool, 1)
        dt_pool_inputs = dt_history.permute(0, 2, 1).permute(0, 2, 1)  # (1, hpool, 1)
        ...
        spatio_mask = torch.zeros(
            (1, self.graph_size + 1, K_SIZE), dtype=torch.bool
        ).to(self.device)
    ...
    def run_episode(self, episode_number):
        ...
        for step in range(EPISODE_STEPS):
            ...
            dt_pool_inputs = dt_history.permute(0, 2, 1).permute(0, 2, 1)
            ...
            temporal_mask = torch.tensor([(len(route) - 1) // HISTORY_STRIDE + 1])
